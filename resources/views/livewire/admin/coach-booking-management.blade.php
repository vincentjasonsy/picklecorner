...
<div class="space-y-8">
    <div class="flex flex-col gap-4 lg:flex-row lg:flex-wrap lg:items-end lg:justify-between">
        <div>
            <h1 class="font-display text-2xl font-bold text-zinc-900 dark:text-white">Coach bookings</h1>
            <p class="mt-1 max-w-2xl text-sm text-zinc-600 dark:text-zinc-400">
                Bookings where a <strong>coach is assigned</strong> at checkout (public or manual). Use venue and coach
                filters to narrow the list. Open a row for full payment and guest details.
                <span class="block text-zinc-500">
                    CSV export of all bookings (not coach-only) is on
                    <a href="{{ route('admin.bookings.index') }}" wire:navigate class="font-semibold text-emerald-700 underline dark:text-emerald-400">Booking history</a>.
                </span>
            </p>
            <p class="mt-2 text-sm text-zinc-600 dark:text-zinc-400">
                ...
                · {{ number_format($rangeBookingCount) }} coach booking{{ $rangeBookingCount === 1 ? '' : 's' }}
            ...
            >
                ...
                @if ($dayBookings->isEmpty())
                    <p class="px-4 py-8 text-center text-sm text-zinc-500 dark:text-zinc-400">
                        No coach bookings in this range{{ $coach !== '' || $venue !== '' ? ' with the current filters' : '' }}.
                    </p>
                @else
                                ...
                                @foreach ($dayBookings as $b)
                                    <tr wire:key="cb-{{ $b->id }}">
                                        ...
                                        <td class="max-w-[12rem] px-4 py-3 text-zinc-800 dark:text-zinc-200">
                                            <span class="block truncate font-medium">{{ $b->coach?->name ?? '—' }}</span>
                                            @if ($b->coach?->email)
                                                <span class="block truncate text-xs text-zinc-500">{{ $b->coach->email }}</span>
                                            @endif
                                        </td>
                                        <td class="max-w-[12rem] px-4 py-3 text-zinc-800 dark:text-zinc-200">
                                        ...
                                        <td class="whitespace-nowrap px-4 py-3 text-right text-zinc-600 dark:text-zinc-400">
                                            @if ($b->coach_fee_cents !== null)
                                                {{ Money::formatMinor($b->coach_fee_cents, $b->currency) }}
                                            @else
                                                —
                                            @endif
                                        </td>
