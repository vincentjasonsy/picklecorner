...
<div class="space-y-8">
    <div class="flex flex-col gap-4 lg:flex-row lg:flex-wrap lg:items-end lg:justify-between">
        <div>
            ...
            <p class="mt-1 text-sm text-zinc-600 dark:text-zinc-400">
                ...
                <span class="text-zinc-500">· {{ $tz }} · max {{ $maxHistoryRangeDays }} days</span>
            </p>
        </div>
        <div class="flex flex-wrap items-center gap-2">
            ...
            <a
                href="{{ route('venue.reports.export.bookings', ['from' => $rangeStart->toDateString(), 'to' => $rangeEnd->toDateString()]) }}"
    ...
    <div class="flex flex-col gap-4 rounded-xl border border-zinc-200 bg-white p-4 dark:border-zinc-800 dark:bg-zinc-900 sm:flex-row sm:items-end">
        ...
        <p class="text-xs text-zinc-500 dark:text-zinc-400 sm:pb-2">
            Long ranges are trimmed to {{ $maxHistoryRangeDays }} days from the start date.
        </p>
    </div>
                                ...
                                @foreach ($dayBookings as $b)
                                    <tr wire:key="vb-{{ $b->id }}">
                                        ...
                                        <td class="whitespace-nowrap px-4 py-3 text-right text-zinc-600 dark:text-zinc-400">
                                            {{ Money::formatMinor($b->amount_cents, $b->currency ?? $courtClient->currency) }}
                                        </td>
                                        <td class="whitespace-nowrap px-4 py-3 text-right text-zinc-600 dark:text-zinc-400">
                                            {{ Money::formatMinor((int) ($b->platform_booking_fee_cents ?? 0), $b->currency ?? $courtClient->currency) }}
                                        </td>
